...
- Task `p5-ws2-iter-a-moc-note-type` (commits `799bcdf`, `901e6d8`; `prd.json` commit field tracks `901e6d8`): `NoteType` enum (NOTE/MOC/DAILY/MEETING/DECISION), `notes.note_type` column + `migrations/010_note_note_type.py`; API create/update + serialization; `NoteDetailView` MOC banner + TOC from outgoing links; tests in `tests/test_models.py`, `tests/test_api.py`, `ui/src/views/NoteDetailView.test.jsx`. Validation: `cd ui && npm test -- --testPathPattern=NoteDetail --run` → 3 passed; `PYTHONPATH=. python3 -m pytest tests/test_models.py tests/test_api.py -q` → 62 passed.
