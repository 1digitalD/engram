...
- Task `p4-ws3-iter-a-rollup-service`: `services/rollup.py` — `rollup_project_to_area(project_id)` loads project notes (M2M + scalar `project_id`), calls `Summarizer.summarize_notes` (Claude), writes an `AREAS`-bucket note on the parent area with body tags `#retrospective` `#project-complete` and `Tag` rows, sets `project.is_archived`, commits, optional embedding queue when Flask app context exists. Tests: `tests/test_rollup.py`. `tests/conftest.py` ignores `test_mcp_server.py` so `pytest tests/ -k rollup` collects without optional `fastmcp`. Validation: `PYTHONPATH=. python3 -m pytest tests/ -k rollup` → 3 passed.

- Task `p4-ws3-iter-b-archive-project-flow`: PATCH `/api/v1/projects/<id>` returns 409 `rollup_confirmation_required` when `is_archived=true` and project has `area_id` unless `rollup_confirmed` is sent; confirmed path runs `rollup_project_to_area` and returns `rollup.note_id`. `ProjectFocus.jsx` — Complete Project modal (rollup copy vs simple archive), `loadAll` + navigate to parent area or `/projects`. `apiRequest` attaches `err.body` for API errors. Tests: `tests/test_api.py` (3 archive cases), `ui/src/views/ProjectFocus.test.jsx`. Validation: `pytest tests/test_phase1_backend_foundation.py tests/test_models.py tests/test_api.py` → 63 passed; `cd ui && npm test -- --testPathPattern=ProjectFocus` → 2 passed; `npm run build` green (static artifacts reverted before commit).

- Task `p4-ws3-iter-c-retrospective-template`: `Summarizer.summarize_project_retrospective` — Claude prompt aligned to PLAN (accomplished / key decisions / lessons / outstanding); chunked note excerpts via `_build_note_chunks`; `_merge_retrospective_partials` when multiple batches. `services/rollup.py` — `_retrospective_body_markdown` with `## What was accomplished`, `## Key decisions`, `## Lessons learned`, `## Outstanding items`, tags unchanged. Tests: `tests/test_rollup.py` asserts section headings + mock JSON shape. Validation: `PYTHONPATH=. python3 -m pytest tests/ -k rollup` → 5 passed. Committed `a71fd8c`.

- Task `p5-ws1-iter-a-graph-node-types`: Graph view — Resource nodes as squares; INBOX notes whose body starts with `# Daily — ` render as calendar icon; note↔note links from `GET /api/v1/links` with colors per `link_type` (related purple, child_of blue, depends_on orange, mentions grey; see_also neutral); stroke width scales with link `weight`. `linksAPI.list`, `graphUtils.js`, `Graph.test.jsx`. Backend: `list_links` in `api/links.py`; test `test_api_v1_links_list`. Validation: `cd ui && npm install && npm test -- --testPathPattern=Graph` → 7 passed; `pytest tests/test_api.py::test_api_v1_links_list` → 1 passed; `npm run build` green. Static assets not committed. Committed `104f813`.

- Task `p5-ws1-iter-b-graph-clustering`: `Graph.jsx` / `Graph.module.css` — “Cluster by” control (None | Project | Area | Tag); convex hull paths per cluster tinted to project/area/tag color (`fill-opacity` / `stroke-opacity`); custom `d3.force` pull toward cluster centroids when clustering is enabled. `graphUtils.js`: `clusterAppearanceForGraphNode`, `convexHullMonotone`, `hullPathFromXY`, `coerceHexColor`, `colorFromKey`. Tags from `useStore` for tag hull colors. Validation: `cd ui && npm test -- --testPathPattern=Graph` → 11 passed. Committed `b8d5014`.

- Task `p5-ws1-iter-c-graph-heatmap` (commit `109e65f`): `Graph.jsx` / `Graph.module.css` — “Activity heat map” checkbox in graph toolbar; note/daily node radius scales with `noteActivityForHeatMap` (API `backlink_count` or incoming `graphLinks`); fill/stroke interpolate grey → note accent (`#7C6AFF`); `d3.forceCollide` radii scale with heat. `graphUtils.js`: `incomingKnowledgeBacklinkCount`, `noteActivityForHeatMap`, `heatMapNodeColors`, `heatMapRadiusScale`, `maxNoteHeatActivity`. Tests in `Graph.test.jsx`. Validation: `cd ui && npm test -- --testPathPattern=Graph` → 13 passed.

- Task `p5-ws1-iter-d-graph-search-filter`: Graph filter sidebar — node-type checkboxes; project/area multi-select (none selected = no restriction; both dimensions use AND when both have selections). Search-by-label with highlight ring + programmatic zoom (`d3.zoomIdentity`, guarded for jsdom). Focus mode: when a node is selected, graph shows that node and 1-hop neighbors on the current filtered link set. Pure helpers in `graphUtils.js`: `GRAPH_ENTITY_TYPES`, `noteProjectIds`, `graphNodeMatchesEnabledTypes`, `graphNodeMatchesLocationFilter`, `neighborIdsForGraphLinks`. Validation: `cd ui && npm test -- --testPathPattern=Graph` → 16 passed; `npm run build` green (static artifacts not committed).

- Task `p5-ws2-iter-b-moc-ai-suggestions` (commit `74e72a7`): `NoteType` enum + `notes.note_type` (default NOTE), migration `010_note_note_type.py`; `services/moc.py` `generate_map_of_content` calls Claude for JSON title/overview/sections, assembles markdown (`#` title, `##` sections, `#moc`), creates `Note` with `note_type=MOC`, `Link` rows `child_of` from MOC → each source (`source=llm`); `api/moc.py` `POST /api/v1/moc/generate`. Tests: `tests/test_api.py` (`-k moc`). Validation: `pytest tests/test_api.py -k moc -q` → 3 passed; `cd ui && npm install && npm run build` green (chunk warning only); `static/` source-only.
- 2026-05-09: Repaired p5-ws2-iter-a integration regression caused by stale Cursor whole-file finalize churn. Restored MOC header/TOC UI from 799bcdf on overhaul/full-implementation-loop. Validation: `cd ui && npm test -- --testPathPattern=NoteDetailView` passed (3 tests); `source .venv/bin/activate && PYTHONPATH=. python3 -m pytest tests/test_models.py tests/test_api.py -q` passed (63 tests).
