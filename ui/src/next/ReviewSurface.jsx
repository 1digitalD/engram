...
export default function ReviewSurface() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const nextId = fromQuery && rows.some((row) => row.id === fromQuery)
        ? fromQuery
        : rows[0]?.id || null;
  ...
  async function afterResolve(reportId) {
  ...
  async function handleAcceptRest() {
    ...
    try {
      ...
      await afterResolve(activeReportId);
    } catch (err) {
    ...
    <div className={styles.surface}>
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>
          Distillation reports — verify proposals, edit inline, or accept the remainder in one batch.
        </p>
      </header>
          ...
          {loadingDetail ? (
            ...
            <article className={styles.report} aria-label="Distillation report">
              ...
              {sections.map((section) => {
                const items = (section.items || []).filter((item) => item.kind !== 'routing_summary' || section.name === 'routing_summary');
