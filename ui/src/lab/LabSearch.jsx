...
export default function LabSearch({ open, onClose }) {
  const navigate = useNavigate();
  const [query, setQuery] = useState('');
  const [selectedIndex, setSelectedIndex] = useState(0);
  const inputRef = useRef(null);
  const { results, loading, error } = useLabSearch(query);
  ...
  return (
    <Sheet open={open} onClose={onClose} ariaLabel="Lab search" mobileBottomSheet>
