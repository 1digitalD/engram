...
describe('V4Today', () => {
  ...
  it('renders today cockpit sections with detail links', async () => {
    v4API.today.mockResolvedValue(mixedTodayPayload);
    ...
    // Expand the low-signal sections to verify their contents are still rendered.
    fireEvent.click(screen.getByRole('button', { name: /Projects without open tasks/i }));
    fireEvent.click(screen.getByRole('button', { name: /Stale projects/i }));
    fireEvent.click(screen.getByRole('button', { name: /Suggested archival/i }));
    fireEvent.click(screen.getByRole('button', { name: /Recent notes/i }));

    expect(screen.getByText('Needs next task')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Stale projects/i })).toHaveAttribute('aria-expanded', 'true');
    ...
    expect(screen.getByRole('button', { name: /Suggested archival/i })).toHaveAttribute('aria-expanded', 'true');
  ...
  it('collapses low-signal sections by default', async () => {
    v4API.today.mockResolvedValue(mixedTodayPayload);
    ...
    expect(await screen.findByText('10 items need your attention today.')).toBeInTheDocument();

    const idleButton = screen.getByRole('button', { name: /Projects without open tasks/i });
    const staleButton = screen.getByRole('button', { name: /Stale projects/i });
    const archivalButton = screen.getByRole('button', { name: /Suggested archival/i });
    const notesButton = screen.getByRole('button', { name: /Recent notes/i });

    expect(idleButton).toHaveAttribute('aria-expanded', 'false');
    expect(staleButton).toHaveAttribute('aria-expanded', 'false');
    expect(archivalButton).toHaveAttribute('aria-expanded', 'false');
    expect(notesButton).toHaveAttribute('aria-expanded', 'false');

    // Content is not rendered (rather than merely aria-hidden).
    expect(screen.queryByText('Needs next task')).not.toBeInTheDocument();
    expect(screen.queryByText('Quietly stalled project')).not.toBeInTheDocument();
    expect(screen.queryByText('Long-forgotten project')).not.toBeInTheDocument();
    expect(screen.queryByText('Recent note')).not.toBeInTheDocument();

    // Expanding reveals the section contents.
    fireEvent.click(idleButton);
    expect(screen.getByText('Needs next task')).toBeInTheDocument();
    expect(idleButton).toHaveAttribute('aria-expanded', 'true');
  });
});
